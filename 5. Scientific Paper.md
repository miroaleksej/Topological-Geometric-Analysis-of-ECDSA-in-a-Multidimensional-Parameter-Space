...
---
**Supported by the Russian Science Foundation (Grant No. 22-11-00101).**
```

### Key Translation Notes:
1. **Mathematical Notation**:
   - All formulas converted to GitHub-compatible `$...$` and `$$...$$` syntax
   - Complex symbols preserved: $\partial$, $\int$, $\mathbb{E}$, $\Lambda$, etc.
   - Subscripts standardized: $\text{Kr}$ for Кронекера, $\text{can}$ for каноническая

2. **Technical Terminology**:
   - "Высота Кронекера" → "Kronecker height"
   - "Наивная высота" → "naive height"
   - "Каноническая высота" → "canonical height"
   - "Торе" → "torus"
   - "Кручение" → "torsion"

3. **Structural Improvements**:
   - Image alt text added: `alt="C vs √h_Kr(G) for ECDSA curves"`
   - Table formatting optimized for GitHub Markdown
   - Consistent spacing around formulas

4. **Scientific Integrity**:
   - All asymptotic notation preserved: $O(1)$, $o(1)$
   - Hypothesis statements translated verbatim
   - Numerical results unchanged from source
   - Grant information accurately converted

The translation strictly adheres to the original mathematical content with no additions or modifications to theoretical claims. All formulas are fully compatible with GitHub's math rendering system.
